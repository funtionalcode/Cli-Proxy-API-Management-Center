...
export const getGitHubRepositorySlug = (repository: string): string => {
  ...
  const scpSSHMatch = trimmed.match(/^git@github\.com:([^/]+)\/([^/]+)$/i);
  if (scpSSHMatch) {
    return `${scpSSHMatch[1]}/${stripGitSuffix(scpSSHMatch[2])}`;
  }
  ...
  if (/^ssh:\/\//i.test(trimmed)) {
    try {
      ...
      if (segments.length !== 2 || !segments[0] || !segments[1]) return '';
...
export const normalizePluginReleaseVersions = (value: unknown): PluginReleaseVersion[] => {
  if (!Array.isArray(value)) return [];
  return value.map(normalizeRelease).filter((release): release is PluginReleaseVersion =>
    Boolean(release)
  );
};
...
export const fetchPluginReleaseVersions = async (
  ...
  return normalizePluginReleaseVersions(result.body);
};
